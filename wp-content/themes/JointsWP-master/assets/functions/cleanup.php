...
function joints_excerpt_more($more)
{
    ...
    return '<a class="excerpt-read-more" href="' . get_permalink($post->ID) . '" title="' . __('Read',
        'jointswp') . get_the_title($post->ID) . '">' . __('... Read more &raquo;', 'jointswp') . '</a>';
}
...
function joints_get_the_author_posts_link()
{
    ...
    $link = sprintf(
        ...
        esc_attr(sprintf(__('Posts by %s', 'jointswp'), get_the_author())),
